...
from src.core.config import TOKENIZER_ENCODING, COMPRESSION_SCORE_THRESHOLD
...
class Compressor:
    ...
    def compress(documents: List[str], query: str, max_tokens: int = 500) -> str:
        ...
        # 1. Semantic scoring using shared embedding model
        import numpy as np
        semantic_scores = [0.0] * len(all_segments)
        try:
            ...
            if emb_model is not None:
                ...
                for i, seg_emb in enumerate(segment_embs):
                    similarity = np.dot(query_emb, seg_emb) / (
                        np.linalg.norm(query_emb) * np.linalg.norm(seg_emb) + 1e-9
                    )
                    semantic_scores[i] = float(similarity)
        except Exception as e:
            logger.warning(f"Semantic scoring in compressor failed: {e}")
        ...
        for i, segment in enumerate(all_segments):
            ...
            lexical_overlap = len(query_words & segment_words) / (len(query_words) + 1)
            
            # Hybrid score is the maximum of semantic similarity and lexical overlap
            hybrid_score = max(lexical_overlap, semantic_scores[i])
