...
from src.core.agent import count_tokens, SYSTEM_PROMPT, live_web_search
from src.core.scraper import scrape_web_page, scrape_web_page_async
from src.core.tools import get_current_time, evaluate_math
        ...
        try:
            if tool_name == "web_search":
                if tool_arg in search_cache:
                    ...
                    observation = live_web_search(tool_arg)
                ...
                logger.info(f"Executing query_sales_db for: {tool_arg}")
                from src.core.sales_db import execute_read_only_sql
                observation = execute_read_only_sql(tool_arg)
            else:
                observation = (
                    f"Error: Unknown tool '{tool_name}'. "
                    "Available tools are: web_search[query], get_system_stats[], get_registry[], direct_response[response], web_scrape[url], get_current_time[], calculator[expression], or query_sales_db[sql_query]."
                )
